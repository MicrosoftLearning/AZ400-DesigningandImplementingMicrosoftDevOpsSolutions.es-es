...
1. En el panel **Crear nueva fuente**, en el cuadro de texto **Nombre**, escribe **`eShopOnWebShared`**. En la sección **Visibilidad**, selecciona Personas específicas y, en la sección **Ámbito**, selecciona la opción **Project:eShopOnWeb**, deja la otra configuración con sus valores predeterminados y haz clic en **Crear**.
...
1. En el panel **Crear un nuevo proyecto**, usa el cuadro de texto de búsqueda para buscar la plantilla **Biblioteca de clases**, selecciona la plantilla de C# que tenga como destino .NET o .NET Standard y haz clic en **Siguiente**.
...
1. Ejecuta lo siguiente para publicar el paquete en la fuente **eShopOnWebShared**. Reemplaza el origen por la dirección URL que copiaste anteriormente de la dirección URL `https://pkgs.dev.azure.com/Azure-DevOps-Org-Name/_packaging/eShopOnWebShared/nuget/v3/index.json` de **origen** de Visual Studio
   ...
   > **Nota**: si el aviso no aparece o recibes la advertencia **advertir: el proveedor de credenciales del complemento no pudo adquirir credenciales. La autenticación puede requerir una acción manual. Considera la posibilidad de volver a ejecutar el comando con --interactive para `dotnet`, /p:NuGetInteractive=true para MSBuild o quitar el modificador -NonInteractive para NuGet"**, puedes agregar el parámetro **--interactive** al comando.
...
1. En la misma ventana de PowerShell usada en la tarea anterior para insertar el nuevo paquete, vuelve a la carpeta **eShopOnWeb.Shared** (`cd ../..`) y ejecuta el siguiente comando **dotnet** para instalar el paquete de ejemplo:
...
Al crear la fuente de paquete de Artifacts de Azure DevOps, por diseño, se permiten **orígenes ascendentes**, como nuget.org en el ejemplo de dotnet que incorpora el paquete Newtonsoft.Json desde donde se hospeda el paquete. Se trata de una práctica común para evitar la duplicación de paquetes y para asegurarse de que siempre se usa la versión más reciente.
