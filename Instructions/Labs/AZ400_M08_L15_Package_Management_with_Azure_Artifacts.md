...
En este ejercicio, queremos recordarte que debes validar los requisitos previos del laboratorio, que tengas lista una organización de Azure DevOps y que hayas creado el proyecto EShopOnWeb. Ve las instrucciones para obtener más detalles.
...
4. Si el sistema pregunta si quieres continuar, haz clic en **Abrir**.
5. Si el sistema te pide que inicies sesión, hazlo con la cuenta de usuario que has usado para configurar tu organización de Azure DevOps.
...
En este ejercicio, aprenderás a trabajar con Azure Artifacts efectuando lo siguiente:
...
2. Con el centro de **Artefactos** en pantalla, haz clic en **+ Crear fuente** en la parte superior del panel.
    ...
    > **Nota**: esta fuente será una colección de paquetes NuGet disponibles para los usuarios de la organización y se almacenará con la fuente de NuGet pública como elemento del mismo nivel. El escenario de este laboratorio estará centrado en el flujo de trabajo para usar Azure Artifacts, por lo que las decisiones de arquitectura y desarrollo reales son meramente ilustrativas.  Esta fuente incluirá funciones comunes que se pueden compartir entre proyectos de esta organización.

3. En el panel **Crear nueva fuente**, en el cuadro de texto **Nombre**, escribe **EShopOnWebShared**. En la sección **Ámbito**, selecciona la opción **Organización**, deja otras opciones con sus valores predeterminados y haz clic en **Crear**.
...
7. En la ventana de Visual Studio, haz clic en el menú **Herramientas**. En el menú desplegable, haz clic en **Administrador de paquetes NuGet** y, en el menú en cascada, haz clic en **Configuración del administrador de paquetes**.
8. En el cuadro de diálogo **Opciones**, haz clic en **Orígenes de paquetes** y haz clic en el signo más para agregar un nuevo origen de paquete.
...
10. Haz clic en **Actualizar** y después haz clic en **Aceptar** para finalizar la adición.

    > **Nota**: Visual Studio ahora está conectado a la nueva fuente.

#### Tarea 2: crear y publicar un paquete NuGet desarrollado de forma interna
...
6. En la interfaz de Visual Studio, en el panel **Explorador de soluciones**, haz clic con el botón derecho en **Class1.cs**. En el menú contextual, selecciona **Eliminar** y, cuando el sistema te pida confirmación, haz clic en **Aceptar**.
...
9. Ve al panel **Conectar a la fuente**, en la sección **NuGet** y selecciona **NuGet.exe**. Aparecerá el panel **NuGet.exe**.
10. En el panel **NuGet.exe**, haz clic en **Obtener las herramientas**.
11. En el panel **Obtener las herramientas**, haz clic en el vínculo **Descargar la versión más reciente de NuGet**. Se abrirá automáticamente otra pestaña del explorador que muestra la página **Versiones de distribución de NuGet disponibles**.
12. En la página **Versiones de distribución de NuGet disponibles**, selecciona **nuget.exe: se recomienda la versión v6.x** más reciente y descarga el archivo ejecutable en la carpeta local **EShopOnWeb.Shared Project** (si mantienes las ubicaciones de carpeta predeterminadas, debe ser C:\EShopOnWeb\EShopOnWeb.Shared).
13. Selecciona el archivo **nuget.exe** y abre sus propiedades haciendo clic con el botón derecho en el archivo y seleccionando **Propiedades** en el menú contextual.
14. En la ventana de contexto Propiedades, en la pestaña **General**, selecciona **Desbloquear** en la sección Seguridad. Para confirmar, pulsa **Aplicar** y **Aceptar**.
...
16. En la ventana **Administrador: Windows PowerShell**, ve a la carpeta EShopOnWeb.Shared ejecutando el siguiente comando:

    ```text
    cd c:\EShopOnWeb\EShopOnWeb.Shared
    ```

    Ejecute lo siguiente para crear un archivo **.nupkg** del proyecto.

    > **Nota**: este es un acceso directo para empaquetar los bits nuGet para la implementación. NuGet es altamente personalizable. Para más información, consulte la [página de creación de paquetes NuGet](https://docs.microsoft.com/nuget/create-packages/overview-and-workflow).
    ...
    > **Nota**: NuGet compila un paquete mínimo basado en la información que puedes identificar del proyecto. Por ejemplo, ten en cuenta que el nombre es **EShopOnWeb.Shared.1.0.0.nupkg**. Ese número de versión se ha recuperado del ensamblado.

17. Después de crear correctamente el paquete, ejecuta lo siguiente para publicar el paquete en la fuente **EShopOnWebShared**:
...
Además de desarrollar tus propios paquetes, ¿por qué no usar la biblioteca de paquetes DotNet de código abierto de Nuget? (https://www.nuget.org) Con unos pocos millones de paquetes disponibles, siempre habrá algo útil para la aplicación.
...
4. El paquete HelloWorld se ha instalado en una subcarpeta **HelloWorld**, en la carpeta EShopOnWeb.Shared. En el **Explorador de soluciones** de Visual Studio, ve al proyecto **EShopOnWeb.Shared** y observa la subcarpeta **HelloWorld**. Haz clic en la flecha pequeña situada a la izquierda de la subcarpeta para abrir la carpeta y la lista de archivos.
5. Observa la subcarpeta **lib**, que tiene un archivo **signature.p7s** como prueba del origen del paquete. Después observa el propio archivo de paquete **HelloWorld.nupkg**.
...
1. En la ventana de PowerShell, ejecuta el comando siguiente:
...
Al crear la fuente de paquetes de Artefactos de Azure DevOps, por diseño, permite **orígenes ascendentes**, como nuget.org en el ejemplo de dotnet. Pero nada bloquea al equipo de DevOps para crear una fuente de paquetes **"solo interna"**.

1. Ve al portal de Azure DevOps, ve a **Artefactos** y selecciona la fuente **EShopOnWebShared**.
2. Haz clic en **Buscar orígenes ascendentes**.
...
7. Haz clic en el engranaje para abrir **Configuración de la fuente**. En la página Configuración de la fuente, selecciona **Orígenes ascendentes**.
8. Observa los distintos administradores de paquetes ascendentes para los diferentes lenguajes de desarrollo. Selecciona **Galería de NuGet** en la lista. Presiona el botón **Eliminar** y, después, el botón **Guardar**.

9. Con estos cambios guardados, será posible cargar el paquete **HelloWorld** mediante NuGet.exe desde la ventana de PowerShell; para ello, vuelve a iniciar el siguiente comando:
...
11. En la solución **EShopOnWeb.Shared** de Visual Studio, haz clic con el botón derecho en el proyecto **EShopOnWeb.Shared** y selecciona **Administrar paquetes NuGet** del menú contextual.
12. En la ventana Administrador de paquetes NuGet, confirma que la opción **Origen del paquete** se haya establecido en **EShopOnWebShared**.
...
En este laboratorio, aprendiste a trabajar con Azure Artifacts siguiendo estos pasos:

- Creaste una fuente y te conectaste a ella.
- Creaste y publicaste un paquete de NuGet.
- Importaste un paquete NuGet con desarrollo personalizado.
- Importaste un paquete de NuGet de origen público.
